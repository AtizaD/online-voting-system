    ...
    try {
        ...
        if ($action === 'add_candidate') {
            ...
            $stmt = $db->prepare("
                ...
                VALUES (?, ?, ?, ?, ?, NOW())
            ");
            $stmt->execute([$student['student_id'], $position_id, $election_id, $photo_url, $current_user['id']]);
            
            $candidate_name = $student['first_name'] . ' ' . $student['last_name'];
            logActivity('candidate_add', "Added candidate: {$candidate_name} for position ID {$position_id}", $current_user['id']);
            ...
            logActivity('candidate_edit', "Updated candidate: {$candidate_name}", $current_user['id']);
            ...
            $candidate_name = $candidate['first_name'] . ' ' . $candidate['last_name'];
            logActivity('candidate_remove', "Removed candidate: {$candidate_name}", $current_user['id']);
        ...
        logError("Candidates management error: " . $e->getMessage());
...
try {
    ...
    logError("Candidates manage data fetch error: " . $e->getMessage());
        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                <div class="d-grid gap-2">
                    <a href="index" class="btn btn-outline-primary btn-sm">
                        <i class="fas fa-list me-2"></i>View All Candidates
                    </a>
                    <a href="photos" class="btn btn-outline-info btn-sm">
                        <i class="fas fa-images me-2"></i>Manage Photos
                    </a>
                    <a href="../students/" class="btn btn-outline-success btn-sm">
                        <i class="fas fa-graduation-cap me-2"></i>Manage Students
                    </a>
                    <a href="../positions/" class="btn btn-outline-warning btn-sm">
